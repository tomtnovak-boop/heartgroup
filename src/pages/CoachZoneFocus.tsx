...
export default function CoachZoneFocus() {
  ...
  const handleViewChange = (view: 'participant' | 'coach') => {
    changeView(view);
    if (view === 'participant') {
      logParticipantRedirect('CoachZoneFocus.handleViewChange', { view });
  ...
  return (
    <div className="h-dvh flex flex-col overflow-hidden" style={{ background: '#0a0a0a' }}>
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        <ZoneFocusDashboard participants={participants} isLoading={isLoading} isSessionActive={sessionActive} sessionCode={sessionCode} lobbyProfileIds={lobbyProfileIds} />
      </div>
