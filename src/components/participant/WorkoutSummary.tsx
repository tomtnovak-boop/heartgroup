...
export function WorkoutSummary({ workout, hrHistory, onClose }: WorkoutSummaryProps) {
  ...
  // Prepare HR history data for chart (sample to max 60 points for performance)
  const sampleInterval = Math.max(1, Math.floor(hrHistory.length / 60));
  const chartData = hrHistory
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex items-center justify-between p-4 bg-card border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold">Training beendet!</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Card className="p-4 text-center">
            ...
            <div className="text-2xl font-bold">{formatDuration(workout.duration_seconds)}</div>
            <div className="text-xs text-muted-foreground">Dauer</div>
          </Card>
          <Card className="p-4 text-center">
            <Flame className="w-6 h-6 mx-auto mb-2 text-orange-500" />
            <div className="text-2xl font-bold">{Math.round(workout.total_calories)}</div>
            <div className="text-xs text-muted-foreground">Kalorien</div>
          </Card>
          <Card className="p-4 text-center">
            <Heart className="w-6 h-6 mx-auto mb-2 text-red-500" />
            <div className="text-2xl font-bold">{workout.avg_bpm}</div>
            <div className="text-xs text-muted-foreground">Ø BPM</div>
          </Card>
          <Card className="p-4 text-center">
            <Activity className="w-6 h-6 mx-auto mb-2 text-yellow-500" />
            <div className="text-2xl font-bold">{workout.max_bpm}</div>
        ...
        {dominantZoneInfo && totalZoneTime > 0 && (
          <Card className="p-4" style={{ borderColor: dominantZone.color }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-semibold">Hauptsächlich in Zone {dominantZoneInfo.zone}</div>
                <div className="text-sm text-muted-foreground">
                  {dominantZoneInfo.name} • {Math.round((dominantZone.seconds / totalZoneTime) * 100)}% der Zeit
                </div>
              </div>
            </div>
          </Card>
